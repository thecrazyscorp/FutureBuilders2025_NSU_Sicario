...
// Check if a specific case_id is requested
...
// 2️⃣ Fetch all patient cases for history
...
$doctor_reply = null;
if ($latest_case) {
    $reply_stmt = $conn->prepare("SELECT cr.*, u.full_name AS doctor_name, dp.specialty
        FROM case_replies cr
        JOIN users u ON cr.doctor_id=u.id
        LEFT JOIN doctor_profiles dp ON dp.user_id=u.id
        WHERE cr.case_id=? ORDER BY cr.created_at DESC LIMIT 1");
...
$referral = null;
if ($latest_case) {
    $ref_stmt = $conn->prepare("SELECT c.name, c.phone, c.address FROM case_referrals r JOIN clinics c ON r.clinic_id=c.id WHERE r.case_id=? LIMIT 1");
    ...
    <main class="main">
        ...
        <?php if($latest_case): ?>
            ...
            <div class="patient-info">
                ...
                <p>Submitted on <?= date('d M Y', strtotime($latest_case['created_at'])) ?> • <?= htmlspecialchars($latest_case['patient_id']) ?></p>
            </div>

            <!-- Severity Badge -->
            <span class="badge <?= $latest_case['severity'] ?>"><?= $latest_case['severity'] ?><?php if($latest_case['severity']=='RED') echo ' – Urgent Attention'; ?></span>

            <!-- AI Summary -->
            <div class="card">
                <h3>AI Triage Summary</h3>
                <div class="value"><?= htmlspecialchars($latest_case['ai_summary'] ?: 'No AI summary available yet.') ?></div>
            </div>
            ...
            <div class="card">
                ...
                <div class="doctor-meta">
                    Dr. <?= htmlspecialchars($doctor_reply['doctor_name']) ?> • <?= htmlspecialchars($doctor_reply['specialty']) ?>
                </div>
                ...
                <div class="value"><?= htmlspecialchars($doctor_reply['treatment_advice'] ?: 'No advice provided.') ?></div>
            </div>
            <?php else: ?>
                <div class="card">
                    <h3>Doctor’s Response</h3>
        ...
        <?php else: ?>
            <div class="card">
                <h3>No Cases Yet</h3>
                <div class="value">
                    You have not submitted any cases yet. Click <a href="patient.php">here</a> to submit your first case.
                </div>
            </div>
        <?php endif; ?>
